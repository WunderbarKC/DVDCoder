...
class SemanticOnly(BaseStrategy):
    def __init__(
        self,
        k: int = 3,
        t: int = 5,
...
""",
        ...
        for example_no, example in enumerate(response["problem"], start=1):
            ...
            confidence = int(str(verification_res['confidence']).strip())
            ...
            consistency_prompt = (
                ...
                f"## Planning:\n{planning}\n"
                f"## Sample IO:\n{self.get_sample_io_str(item['sample_io'])}\n"
                f"Generate a hypothetical implementation based on this plan, then judge whether typical generated code would faithfully implement the logic.\n"
                f"<root><alignment>Yes/No</alignment><explanation>...</explanation></root>"
            )
            ...
            alignment_score = 1 if alignment_flag == "yes" else 0
            ...
            counterfactual_prompt = (
                ...
                f"Would a code based on the above planning need significant changes?\n"
                f"<root><conflict>Yes/No</conflict><rationale>...</rationale></root>"
            )
            ...
            conflict_score = 0 if conflict_flag == "yes" else 1  # no conflict → more robust plan

            # 汇总指标（可扩展为加权和）
            final_score = 0.5 * confidence + 0.3 * alignment_score + 0.2 * conflict_score
