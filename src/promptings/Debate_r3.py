...
""",
        ...
        for example_no, example in enumerate(response["problem"], start=1):
            ...
            # )
            # 准备基础 prompt
            base_content = input_for_problem_planning[0]['content']  # 原问题+示例+算法+样例IO 等
            ...
            # 存储各轮各代理的计划
            plans = {agent: [] for agent in agents}
            ...
            for round_no in range(1, MAX_ROUNDS + 1):
                for agent, instruction in agents.items():
                    if round_no == 1:
                        ...
                        # 第二及以后：参考其他所有代理上轮的 plan
                        others = [
                            f"{other}: {plans[other][-1]}"
                            for other in agents if other != agent
                        ]
                        others_str = "\n".join(others)
                        prompt = (
                            f"{base_content}\n\n"
                            f"Below are the latest proposals from other agents:\n"
                            f"{others_str}\n\n"
                            f"-- {agent}: {instruction}\n"
                            f"Please revise and improve your previous plan accordingly."
                        )
                    # 调用模型
                    plan, _, _ = self.gpt_chat([{"role": "user", "content": prompt}])
                    plans[agent].append(plan)

                # 如果想在某轮提前停止，可以在这里加判断（例如三者 plan 都相同）
                # if all(plans[a][-1] == plans["UA"][0] for a in agents):
                #     break

            # 三轮后，汇总最新的三个 plan，生成最终综合版
            summary_lines = [
                f"{agent}: {plans[agent][-1]}"
                for agent in agents
            ]
            debate_summary = (
                "Below are the final proposals from the three agents after debate rounds:\n\n"
                + "\n\n".join(summary_lines)
                + "\n\nPlease synthesize these into a single, executable comprehensive plan:"
            )
