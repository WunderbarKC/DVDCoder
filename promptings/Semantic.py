...
""",
        ...
        for planning_with_ex in plannings:
            ...
            consistency_prompt = (
                ...
                f"## Code:\n{code}\n"
                f"----------------------\n"
                f"Please return an XML response with the following format:\n"
                f"<root>\n"
                f"<alignment>Yes/No</alignment>\n"
                f"<explanation>Explain if and where the code deviates from the planning.</explanation>\n"
                f"</root>"
            )
            ...
            print("Input for planning-code semantic alignment check: ")
            print(consistency_prompt, flush=True)

            alignment_res, pr_tok_2, com_tok_2 = self.gpt_chat([{"role": "user", "content": consistency_prompt}])
            item["api_calls"] += 1
            pr_tok += pr_tok_2
            com_tok += com_tok_2
            ...
            alignment_flag = alignment_res.get("alignment", "").strip().lower()
            alignment_explanation = alignment_res.get("explanation", "").strip()

            print("Response from semantic alignment check:")
            print(alignment_res, flush=True)

            # 如果未对齐，进入反事实检测机制
            if alignment_flag != "yes":
                # 构造反事实情境：稍微修改 planning 的某一部分
                counterfactual_prompt = (
                    f"You are a counterfactual detector.\n"
                    f"The following planning is slightly changed from the original to test whether the code reacts appropriately.\n"
                    f"## Original Planning:\n{planning}\n"
                    f"## Modified Planning (Counterfactual):\nPlease assume the input format is changed or edge cases must be handled differently.\n"
                    f"## Code:\n{code}\n"
                    f"----------------------\n"
                    f"Does the current code address the changes introduced in the counterfactual planning?\n"
                    f"Return the result in this format:\n"
                    f"<root>\n"
                    f"<conflict>Yes/No</conflict>\n"
                    f"<rationale>Explain your judgment.</rationale>\n"
                    f"</root>"
                )
                counterfactual_prompt = self.replace_tag(counterfactual_prompt, "conflict")
                counterfactual_prompt = self.replace_tag(counterfactual_prompt, "rationale")

                print("Input for counterfactual check: ")
                print(counterfactual_prompt, flush=True)

                cf_result, pr_tok_3, com_tok_3 = self.gpt_chat([{"role": "user", "content": counterfactual_prompt}])
                item["api_calls"] += 1
                pr_tok += pr_tok_3
                com_tok += com_tok_3

                cf_result = self.parse_xml(cf_result)
                conflict_flag = cf_result.get("conflict", "").strip().lower()
                rationale = cf_result.get("rationale", "").strip()

                print("Response from counterfactual check:")
                print(cf_result, flush=True)

                # 如果反事实测试也失败，则触发自动改进逻辑（比如添加 planning 注释提示，要求更精细）
                if conflict_flag == "yes":
                    print("Counterfactual mismatch detected. Code may require major improvement.")
                    # 可在此处自动进入重新生成 planning/code 逻辑，或打标签退出
            passed = False
